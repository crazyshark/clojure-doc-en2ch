...
一般情况下，collection functions (conj,assoc,dissoc,and so on)支持保存元数据(metadata)，序列函数(cons,take,drop,etc.) 不支持。
但是也有一些例外，在clojure 1.0,conj函数对向量(vector)处理时是不保存元数据(metadata)(这是个bug),但是在clojure1.1 时，又支持了。
(The moral is this) ,小心处理带有元数据的数据结构。不要设想元数据会被存储。测试是第一位的。
...
警告
-----------------------
元数据是clojure与从不同的特性。其他编程语言很少类似的特性。当你考虑使用元数据（metadata)，要小心它代表的语义：元数据（metadata）
